...
/**
 * Imports virtual products (e.g. giftcards) with custom options
 * (e.g. recipient name, personal message, delivery date).
 *
 * Reads `_files/base/virtual_products.csv` whose rows declare
 * `type_id=virtual` and an optional `custom_options` column. The
 * importer handles those fields directly — this fixture is mostly
 * a thin shim that picks up translations after each row saves.
 */
class VirtualProductFixture extends AbstractCsvFixture
{
    ...
    {
        return 'base/virtual_products.csv';
    }
